...
export function QuickActionsGrid({ actions, onActionClick }: QuickActionsGridProps) {
  ...
  const getCategoryColor = (category: QuickAction['category']) => {
    switch (category) {
      ...
      case 'navigation':
        return 'bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-100 border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:hover:bg-amber-900/30'
      case 'tasks':
